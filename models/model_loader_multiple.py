...
def load_model(model_choice):
    ...
    if model_choice == 'qwen':
        ...
        # Add support for multi-image processing
        def process_multiple_images(images):
            """
            Process multiple images and combine their embeddings.
            """
            image_embeddings = []
            for image in images:
                inputs = processor(image, return_tensors="pt").to(device)
                embedding = model.get_image_embedding(**inputs)
                image_embeddings.append(embedding)

            # Combine embeddings (e.g., concatenation along the feature dimension)
            combined_embedding = torch.cat(image_embeddings, dim=1)
            return combined_embedding

        # Attach the custom multi-image processing function to the model
        model.process_multiple_images = process_multiple_images

        _model_cache[model_choice] = (model, processor, device)
        logger.info("Llama-Vision model (4-bit quantized) with multi-image support loaded and cached.")
        return _model_cache[model_choice]
